...
describe("SyncChain", () => {
  describe(".constructor", () => {
    ...
    it("flattens a resolved value that is a Promise", async () => {
      const p = SyncChain.eager<number>((res) => {
        res(Promise.resolve(Promise.resolve(Promise.resolve(1))));
      });
  ...
  describe(".resolve", () => {
    it("resolves with the given value", () => {
      ...
      expect(SyncChain.resolve(-0).value).toBe(-0);
      expect(SyncChain.resolve(Number.POSITIVE_INFINITY).value).toBe(
        Number.POSITIVE_INFINITY,
      );
      expect(SyncChain.resolve(Number.NEGATIVE_INFINITY).value).toBe(
        Number.NEGATIVE_INFINITY,
      );
      expect(SyncChain.resolve(Number.NaN).value).toBe(Number.NaN);
      expect(SyncChain.resolve(-Number.NaN).value).toBe(-Number.NaN);
    ...
    it("flattens a nested SyncChain", () => {
      const p = SyncChain.resolve(
        SyncChain.resolve(
          SyncChain.resolve(SyncChain.resolve(SyncChain.resolve(1))),
        ),
      );
      expect(p.get()).toBe(1);
    });
    ...
    it("rejects a SyncChain that rejects", () => {
      const p = SyncChain.resolve(SyncChain.reject("one"));
      expect(p.run().error).toBe("one");
    });

    it("flattens a nested Promise", async () => {
      const p = SyncChain.resolve(SyncChain.resolve(Promise.resolve(1)));
      expect(await p.run().promise).toBe(1);
    });

    it("rejects a Promise that rejects", async () => {
      const p = SyncChain.resolve(Promise.reject("one"));
  ...
  describe(".reject", () => {
    ...
    it("flattens a nested SyncChain", () => {
      ...
      const q = SyncChain.resolve(SyncChain.reject(SyncChain.resolve(1)));
      expect(q.run().error).toBe(1);

      const r = SyncChain.reject(
        SyncChain.reject(SyncChain.reject(SyncChain.reject(1))),
      );
      expect(r.run().error).toBe(1);
    });

    it("flattens a nested Promise", async () => {
      const p = SyncChain.reject(Promise.resolve("one"));
      await expect(p.run().promise).rejects.toThrow("one");

      const q = SyncChain.reject(Promise.reject("two"));
  ...
  describe(".catch", () => {
    it("converts a rejection to a fulfillment", () => {
      const p = SyncChain.reject(1).catch((error) => error);
      expect(p.error).toBeUndefined;
      expect(p.value).toBe(1);
    ...
    it("stores a thrown error in .error", () => {
      const p = SyncChain.reject(1).catch(() => {
        throw 2;
      });
      expect(p.run().error).toBe(2);
    });
  ...
  describe(".finally", () => {
    it(".finally adds an operation without modifying the settled value", () => {
      let count = 0;
      const p = SyncChain.resolve(1).finally(() => {
        ++count;
      });
      expect(p.get()).toBe(1);
      expect(count).toBe(1);

      const q = SyncChain.reject(1).finally(() => {
        ++count;
      });
      expect(q.run().error).toBe(1);
      expect(count).toBe(2);

      const r = SyncChain.resolve(0)
        .finally(() => {
          ++count;
        })
        .then((x) => x + 5);
  ...
  describe(".addEventListener", () => {
    ...
    it("executes a listener immediately if the promise is already settled", () => {
      ...
      count = 0;
      const q = SyncChain.resolve().then(() => {
        ++count;
        throw "error";
      });
      q.addEventListener("rejected", () => {
    ...
    it("supplies the settled value to the listener", () => {
      let result = 0;
      const p = SyncChain.resolve(1);
      p.addEventListener("fulfilled", (value) => {
        result = value;
      });
  ...
  describe(".onfulfilled", () => {
    it("behaves like calling removeEventListener and addEventListener when assigned to", () => {
      let count = 0;
      let listener: () => void;
      const p = SyncChain.lazy((res) => {
        count = 1;
        res();
      });
      p.onfulfilled = listener = () => {
        count += 2;
      };
      expect(count).toBe(0);
  ...
  describe(".any", () => {
    ...
    it("rejects when all of the PromiseLikes reject", () => {
      ...
      rej2("two");
      expect(p.isRejected).toBe(true);
      expect(p.error.errors).toStrictEqual(["one", "two", "three"]);
    });
  });
