...
import { addIDBValue, deserialize, getIDBValue, IDBLocation, OpfsLocation, setIDBValue, serialize, initIDBStores, clearIDBStore, deleteIDBValue, deleteFromStorage } from "../../src/browser-utils/serialization";
...
describe("IndexedDB convenience functions", () => {
  ...
  const { get: get1, put: put1, add: add1, delete: delete1 } =
    storeFunctions(dbName, storeName1);
  const { get: get2, put: put2, add: add2, delete: delete2 } =
  ...
  it("get should return undefined when non-existing value is retrieved", async () => {
    await expect(put1("food", "dinner",
      ["Broiled broccoli", "Shoyu and pepper rice"],
    )).resolves.toBe(true);
    await expect(put1("food", "lunch",
      "Potato salad",
    )).resolves.toBe(false);
    await expect(put1("food", "breakfast",
      ["Omlette", "Peanut butter and jelly sandwich"],
    )).resolves.toBe(false);
  ...
  it("get and put should work with nested values", async () => {
    await expect(
      put1("farewell", "phrases.0", "Goodbye.")
    ).resolves.toBe(true);
    await expect(put1("farewell", "phrases.1", "See you later.")
    ).resolves.toBe(false);
    await expect(put1("farewell", "phrases.2", "Take care.")
    ).resolves.toBe(false);
    ...
    await expect(put1("dialog", "speeches", [
  ...
  it("put should replace and existing value with the same key", async () => {
    await expect(put1("greeting", "first", "Hello."))
      .resolves.toBe(true);
    await expect(put1("greeting", "second", "How are you?"))
      .resolves.toBe(false);
    ...
    await expect(put1("greeting", "first", "Hi!"))
      .resolves.toBe(false);
    expect(await get1("greeting")).toStrictEqual({
      key: "greeting",
      first: "Hi!",
  ...
  it("get and put should work with nested keys", async () => {
    await expect(put2("00:00:00", "time.finished", "00:01:00"))
      .resolves.toBe(true);
    await expect(put2("00:00:00", "task", "Wash hands"))
      .resolves.toBe(false);
    await expect(put2("00:01:00", "time.finished", "00:01:20"))
      .resolves.toBe(true);
  ...
  it("put should always keep the value at the key path equal to key", async () => {
    ...
    await expect(put2("00:02:03", "time", {
      created: "unknown",
      modified: "00:02:10",
    })).resolves.toBe(false);
    expect(await get2("00:02:03")).toStrictEqual({
...
describe("IndexedDB serialization", () => {
  ...
  describe("deleteFromStorage", () => {
    ...
    it("should return true if the specified location has data", async () => {
      ...
      const locationInner = { ...location, valuePath: location.valuePath + ".d.dd" };
