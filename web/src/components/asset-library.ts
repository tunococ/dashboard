...
import { deleteFromStorage, deserialize, IDBLocation, initIDBStores, serialize } from "../browser-utils/serialization";
...
export type MediaAsset = TextAsset | ImageAsset | AudioAsset | VideoAsset | UnknownMediaAsset;
...
export class AssetLibrary extends LitElement {
  ...
  static get styles() {
    return css`
      ...
      *::after {
      ...
      .breadcrumbs-folder {
        background-color: var(--asset-library-breadcrumbs-background-color, #beffa0);
      ...
      .entry-row-focused td {
        border-top: calc(0.05 * var(--srem)) dashed var(--focused-selection-color, #ffffff);
        border-bottom: calc(0.05 * var(--srem)) dashed var(--focused-selection-color, #ffffff);
      ...
      .entry-row-focused:not(.entry-row-selected) td {
        border-top: calc(0.05 * var(--srem)) dashed var(--focused-color, #000000);
        border-bottom: calc(0.05 * var(--srem)) dashed var(--focused-color, #000000);
  ...
  private findScrollIndex(entries: LibraryEntry[], index: number, direction: "up" | "down") {
    if (index < 0) {
    ...
    try {
      ...
      if (direction === "down") {
        ...
        const {
          bottom: nextBottom,
        } = getEntryOffset(index + 1);
        const targetViewBottom = scrollViewBottom +
      ...
      const targetViewTop = scrollViewTop -
        (previousTop < scrollViewTop ? scrollViewHeight : 0);
      let i = index;
      for (; i > 0; --i) {
        const {
  ...
  moveFocus(entries: LibraryEntry[], fromIndex: number, toIndex: number, event: KeyboardEvent) {
  ...
  onKeyDown(
    ...
    switch (event.key) {
      ...
      case "PageUp": {
        const scrollIndex = this.findScrollIndex(entries, focusedEntryIndex, "up");
        if (scrollIndex < 0) {
          return;
        }
      ...
      case "PageDown": {
        const scrollIndex = this.findScrollIndex(entries, focusedEntryIndex, "down");
        if (scrollIndex < 0) {
          return;
        }
        this.moveFocus(entries, focusedEntryIndex, scrollIndex, event);
  ...
  private htmlEntryTable(currentFolder: LibraryFolder, entries: LibraryEntry[], focusedEntryIndex: number) {
    const tableHeader = this.numColumns === 1 ? html`
      <tr>
  ...
  private importFromUrl(
    ...
    try {
      ...
      const entryName = name ? name : getDefaultNameFromUrl(urlObject, currentFolder.entries);
  ...
  private htmlAssetTools(currentFolder: LibraryFolder) {
    ...
    const onClickImportFromUrlButton = (e: Event) => {
      ...
      if (!importFromUrlDialog) {
        return;
      }
      e.preventDefault();
      e.stopPropagation();

      importFromUrlDialog.addEventListener("close", (event: Event) => {
        const { url, name, mimeType } = importFromUrlDialog.dataset;
        if (!url) {
          return;
        }
      ...
      importFromUrlDialog.dataset.url = "";
      importFromUrlDialog.dataset.name = "";
      importFromUrlDialog.dataset.mimeType = "";
  ...
  private htmlSelectOptions() {
    ...
    const okButton = this.selectMode === "none" ? nothing : html`
      <button id="ok-button"
  ...
  private htmlImportFromUrlDialog(currentFolder: LibraryFolder) {
    ...
    const updateInputStates = (active: boolean = true) => {
      ...
      try {
        ...
        nameInput.placeholder = getDefaultNameFromUrl(url, currentFolder.entries);
    ...
    const submit = (ok: boolean) => {
      ...
      if (ok) {
        ...
        dialog.dataset.url = urlInput.value;
        ...
        fetchMimeType(dialog.dataset.url)
          .then(mimeType => {
            dialog.dataset.mimeType = mimeType ?? "";
            dialog.close();
            updateInputStates(true);
          })
          .catch(_error => {
            dialog.dataset.mimeType = "";
            dialog.close();
    ...
    return shtml`
      <dialog
        ...
        data-title="Import asset from URL"
  ...
  render() {
    ...
    const focusedEntryIndex = entries.findIndex(entry => entry === this.focusedEntry);
  ...
  addEventListener(...p: [any, any, any]): void {
    return super.addEventListener(...p);
  }
}
...
function htmlSpinner(height: string = "1lh") {
  return html`
    <svg height="${height}" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="gradient-spinner" x1="0%" y1="0%" x2="100%" y2="0%">
      ...
      <circle class="spinner"
        cx="50" cy="50" r="40"
        fill="none" stroke-width="20" stroke="url(#gradient-spinner)" stroke-linecap="round"
      />
    </svg>
  `;
}
...
function getCollisionFreeName(name: string, existingRecord: Record<string, any>) {
...
function getDefaultNameFromUrl(urlObject: URL, existingRecord: Record<string, any>) {
